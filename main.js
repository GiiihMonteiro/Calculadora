...
function calcularResultado() {
    ...
    } catch (erro) { // Caso ocorra um erro na execução do TRY, o 'catcg' caotura o erro e executa o bloco de instruções definido entre chaves
        tela.value = 'Erro';
    }
}

// ACIONANDO UM EVENTO DENTRO DO JAVASCRIPT por meio de 'addEventListener'
// Aqui, defiinimos um event listener para qualquer digitação no teclado ocorrida no 'document' por meio do evento 'keydown'. Quando o evento é realizado, uma função sem nome que recebe o parâmetro 'evento' é executado
document.addEventListener('keydown', function (evento) {
    if("0123456789./*-+".includes(tecla)) {
        // Caso o IF retorne trrue, chamamos a função 'adicionarValor' passando o valor armazenado em 'tecla' como parâmetro
        adicionarValor(tecla);
    }

    // Aqui, deefinimos que se a tecla digitada foi o backspace, chamamos a função que apaga o último número
    if (tecla === 'Backspace') {
        apagarUltimo();
    }

    // Aqui deefinimos que se a tecla digitada foi o enter, chamamos a função que realiza o cálculo
